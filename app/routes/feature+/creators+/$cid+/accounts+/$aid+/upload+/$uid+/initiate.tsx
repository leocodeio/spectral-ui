...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  const { aid, cid, uid } = params;
  if (!aid || !uid) {
    return {
      success: false,
      origin: "contribute",
      message: "Account ID and Version ID are required",
      data: null,
  ...
  const result = await createVersion(request, uid, formData);

  if (result.success) {
    return redirect(`/feature/creators/${cid}/accounts/${aid}/upload/${uid}`);
  }
  return result;
};
